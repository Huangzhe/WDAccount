...
public class Platform {
    ...
    public List<TenderRecord> getTenderRecordList() {
        if (tenderRecordList == null) {
            if (daoSession == null) {
                throw new DaoException("Entity is detached from DAO context");
            }
            TenderRecordDao targetDao = daoSession.getTenderRecordDao();
            List<TenderRecord> tenderRecordListNew = targetDao._queryPlatform_TenderRecordList(id);
            synchronized (this) {
                if(tenderRecordList == null) {
                    tenderRecordList = tenderRecordListNew;
                }
            }
        }
    ...
    /** Resets a to-many relationship, making the next get call to query for a fresh result. */
    public synchronized void resetTenderRecordList() {
        tenderRecordList = null;
    }
